...
- Install-introspection tools (read_logs, get_config, get_database_schema/info, get_table_counts, get_project_config_diff, get_environment) are locked to admins over read/content-scope tokens; open specific ones with the new `scopedTokenPrivilegedTools` config setting
